...
if language == 'Bahasa Indonesia':
    title = '💼 Kalkulator Estimasi Biaya Kesehatan'
    success_message = '💵 Perkiraan biaya kesehatan tahunan Anda: **${estimated_cost:,.2f}**'  # Bahasa Indonesia message
else:
    title = '💼 Health Cost Estimator'
    success_message = '💵 Your estimated annual health cost: **${estimated_cost:,.2f}**'  # English message
...
if multiple_predictions:
    ...
    if st.button('Calculate All Predictions' if language == 'English' else 'Hitung Semua Prediksi'):
        for i in range(len(ages)):
            ...
            st.write(f"**Prediction for Age {ages[i]}**: ${estimated_cost:,.2f}" if language == 'English' else f"**Prediksi untuk Usia {ages[i]}**: Rp {estimated_cost:,.2f}")
    ...
    if st.button(button_label):
        ...
        estimated_cost = predict_charges(user_input)
        st.success(success_message.format(estimated_cost=estimated_cost))
        ...
        if language == 'English':
            ...
            st.write(f"💡 Your estimated cost after considering region {region}: ${adjusted_cost:,.2f}")
        else:
            region_comparison = {
            ...
            region_cost_factor = region_comparison.get(region, 1.0)
            adjusted_cost = estimated_cost * region_cost_factor
            st.write(f"💡 Perkiraan biaya Anda setelah mempertimbangkan wilayah {region}: Rp {adjusted_cost:,.2f}")
